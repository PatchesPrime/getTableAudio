...
localFile = '/media/databank/Audio/Music/Ambience/'
...
# Do the parsing work.
parser = MyHTMLParser()
# req = urllib.request.Request(urlTA, headers={'User-Agent': 'Mozilla/5.0'})
# tableAudio = urllib.request.urlopen(req)
tableAudio = requests.get(urlTA, headers={'User-Agent': 'Mozilla/5.0'})
try:
    parser.feed(tableAudio.text)
except Exception:  # HTMLParseError removed?
